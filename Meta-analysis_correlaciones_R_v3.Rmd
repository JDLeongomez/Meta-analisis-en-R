...
header-includes: 
  ...
  \lfoot{\footnotesize{{Versión \textbf{3}:} \today}}
...
```{r infplot, fig.height = 4.5, fig.cap = "Diagnóstico de influencia. Estudios particularmente influyentes serían representados con un punto rojo. Los números 1 a 16 en el eje *X* representan cada estudio, como lo definimos en columna \\texttt{study\\_id} de la Tabla \\@ref(tab:estructuramod). En este caso, no hay ningún estudio que se considere demasiado influyente, por lo que este análisis sugiere que no es necesario excluir ningún estudio de nuestro meta-análisis."}
plot(inf.res)
```
...
El código y archivos necesarios para crear este documento, están disponibles en el OSF (https://osf.io/na4p9/).