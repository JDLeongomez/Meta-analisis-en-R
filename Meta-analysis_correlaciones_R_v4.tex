...
\usepackage{setspace} \usepackage{float} \floatplacement{figure}{H} \usepackage[T1]{fontenc} \usepackage[utf8]{inputenc} \usepackage{fancyhdr} \pagestyle{fancy} \lhead{Juan David Leongómez} \rhead{\textit{Meta-análisis de correlaciones y meta-regresión en {R:} Guía práctica}} \rfoot{\footnotesize{{doi:} \href{https://doi.org/10.31222/osf.io/yaxd4}{10.31222/osf.io/yaxd4}}} \lfoot{\footnotesize{{Versión \textbf{4}:} \today}} \renewcommand{\abstractname}{Descripción} \usepackage[spanish]{babel} \usepackage{csquotes} \usepackage[style=apa,backend=biber]{biblatex} \DeclareLanguageMapping{spanish}{spanish-apa} \usepackage{hanging} \usepackage{amsthm,amssymb,amsfonts} \usepackage{tikz,lipsum,lmodern} \usepackage[most]{tcolorbox} \usepackage{multicol} \usepackage{fontawesome5} \usepackage{multirow,booktabs,caption} \renewcommand\spanishtablename{Tabla} \DeclareCaptionLabelSeparator*{spaced}{\\[1ex]} \DeclareCaptionLabelSeparator{point}{. } \captionsetup[table]{labelfont=bf, textfont=it, format=plain, justification=justified, singlelinecheck=false, labelsep=spaced, skip=5pt} \captionsetup[figure]{labelfont=bf, format=plain, justification=justified, singlelinecheck=false,labelsep=point,skip=5pt} \captionsetup[figure]{font=small} \usepackage{orcidlink} \definecolor{iacol}{RGB}{246, 130, 18} \definecolor{iacoldark}{RGB}{246, 127, 17} \newcommand{\opensupplement}{\setcounter{table}{0} \renewcommand{\thetable}{A\arabic{table}} \setcounter{figure}{0} \renewcommand{\thefigure}{A\arabic{figure}}} \newcommand{\closesupplement}{\setcounter{table}{0} \renewcommand{\thetable}{\arabic{table}} \setcounter{figure}{0} \renewcommand{\thefigure}{\arabic{figure}}}
\usepackage{booktabs}
...
El meta-análisis es un método ampliamente utilizado para sintetizar los datos de diferentes estudios. Sin embargo estudiantes, profesionales e investigadores a menudo carecemos de conocimientos prácticos para hacer e interpretar un meta-análisis. Esta guía presenta una variedad de herramientas para realizar meta-análisis de correlaciones en R, mediante el uso de ejemplos reales. Incluye desde análisis simples y su interpretación, hasta el análisis de moderadores (meta-regresión), usando los paquetes \href{https://www.metafor-project.org/doku.php}{\texttt{metafor}} \autocite{viechtbauer2010} y \href{https://cran.r-project.org/web/packages/metaviz/vignettes/metaviz.html}{\texttt{metaviz}} \autocite{KossmeierMetaviz}. También incluye explicaciones para la transformación de coeficientes \emph{r} de Pearson a \emph{z} de Fisher (y viceversa), creación de gráficos de bosque (\emph{forest plots}) y gráficos de embudo (\emph{funnel plots}), análisis de heterogeneidad y diagnósticos de influencia, así como estrategias para detectar posibles sesgos de publicación utilizando el paquete \href{https://www.r-pkg.org/pkg/weightr}{\texttt{weightr}} \autocite{coburnWeightr2019}, y para determinar el poder estadístico de un meta-análisis utilizando \href{https://www.dsquintana.blog/metameta-r-package-meta-analysis/}{\texttt{metameta}} \autocite{quintanaMetameta2022}. Esta guía práctica acompaña el video \href{https://youtu.be/RNw92QdsW4g}{\textbf{Meta-análisis de correlaciones en R}}, de mi canal de YouTube \href{https://www.youtube.com/@InvestigacionAbierta}{\emph{Investigación Abierta}}.
...
\textbf{Conocimientos necesarios:} Esta guía asume (1) un manejo básico de R, suficiente para al menos instalar paquetes y correr funciones; (2) comprensión general de correlaciones y regresiones; y (3) un entendimiento de los principios del meta-análisis. De ser necesario, te recomiendo ver \href{https://youtu.be/ntBbkOn9D_o}{este video} introductorio al meta-análisis en \emph{jamovi} \autocite{leongomezMetaanalysis2021} publicado en mi canal de YouTube \href{https://www.youtube.com/@InvestigacionAbierta}{\emph{Investigación Abierta}}.

\textbf{Versión de R:} Para poder correr el código tal cual aparece en esta guía, se requiere R versión 4.1 o superior, pues he usado el \emph{pipe} nativo de R (\texttt{\textbar{}\textgreater{}}). Para saber qué versión tienes instalada, puedes usar el comando \texttt{R.version.string}. En versiones anteriores de R, el \emph{pipe} nativo puede ser reemplazado por \texttt{\%\textgreater{}\%} que es el \emph{pipe} del paquete \texttt{magrittr} \autocite{magrittrcite} de \texttt{tidyverse} \autocite{Wickhamtidyverse2019}. Para más información, puedes ver la sección \href{https://datanalytics.com/libro_r/1100_programacion.html\#tuber\%C3\%ADas-pipes}{11.1.5} de \textcite{gil2023}.

\textbf{Inspiración:} Esta guía fue originalmente inspirada por el video de \textcite{quintanaHowPerformMetaanalysis2021}, de donde tomé además ejemplos para el análisis básico. Así mismo, la sección \ref{todas-combinaciones}, dedicada al meta-análisis combinatorio, está inspirada y toma ejemplos modificados de \textcite{quintanaOhMyGOSH2020}.
...
Por suerte, existen técnicas poderosas para buscar evidencia de sesgos de publicación en los estudios incluidos en un meta-análisis, y para poder hacer estimaciones corregidas del efecto \emph{real} ---teniendo en cuenta estos sesgos--- para así obtener resultados más precisos.
...
Básicamente, \textcite{molloy2014} estudiaron si existe una asociación entre la escrupulosidad (\emph{conscientiousness}\footnote{En el contexto de uno de los modelos más robustos de la personalidad (\emph{Big Five Model}), la dimensión de la \emph{escrupulosidad} (también llamada conciencia o ``voluntad de logro''), se refiere al auto-control, incluyendo la planificación y ejecución de tareas. Para una definición detallada del concepto de escrupulosidad en este contexto, ver \textcite{johnBigFiveTrait1999}.}) y la adherencia a la medicación. En otras palabras, ¿las personas más \emph{escrupulosas} tienden a cumplir más con la medicación prescrita?
...
\bottomrule
\multicolumn{11}{l}{\rule{0pt}{1em}\textit{Nota:} Datos tomados de Molloy et al. (2014).}\\
\end{tabular}}
\end{table}
...
Como verás, los valores originales de los coeficientes de correlación (\texttt{ri}) son muy similares a los valores \emph{z} de Fisher (\texttt{yi}). Sin embargo, estas diferencias no siempre son tan pequeñasm y pueden tener efectos muy importantes, como explicaré en la sección \ref{r-to-z}, más adelante.
...
Puedes ver el resultado de tu análisis en la consola de R sencillamente usando como comando el nombre que le dimos al objeto al que lo asignamos (en este caso, \texttt{res}):
...
\hypertarget{r-to-z}{%
\subsubsection{\texorpdfstring{Alternativa: reportar el estimado como \emph{r} de Pearson en vez de \emph{z} de Fisher}{Alternativa: reportar el estimado como r de Pearson en vez de z de Fisher}}\label{r-to-z}}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/rvsz-1.pdf}
...
Si has elegido reportar los resultados de tu meta-análisis utilizando coeficientes \emph{r} de Pearson, es importante que mantengas la consistencia al transformar también el error estándar y los límites del intervalo de confianza del 95\%. Ten en cuenta que estos valores están reportados como \emph{z} de Fisher, ya que se utilizó este tamaño de efecto en el meta-análisis. Al hacer esta transformación, podrás presentar tus resultados de manera más clara y precisa.
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/infplot-1.pdf}
...
\FunctionTok{forest}\NormalTok{(res, }
       \AttributeTok{cex =} \FloatTok{0.75}\NormalTok{, }\AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{1.6}\NormalTok{, }\FloatTok{1.6}\NormalTok{),}
       \AttributeTok{slab =} \FunctionTok{paste}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{authors, dat}\SpecialCharTok{$}\NormalTok{year, }\AttributeTok{sep =} \StringTok{", "}\NormalTok{), }\AttributeTok{showweights =} \ConstantTok{TRUE}\NormalTok{,}
       \AttributeTok{xlab =} \StringTok{"Coeficiente de correlación (z de Fisher)"}\NormalTok{, }\AttributeTok{digits =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,3L),}
...
\NormalTok{                           .(}\FunctionTok{formatC}\NormalTok{(res}\SpecialCharTok{$}\NormalTok{QE, }\AttributeTok{digits =} \DecValTok{2}\NormalTok{, }\AttributeTok{format =} \StringTok{"f"}\NormalTok{)), }\StringTok{", p "}\NormalTok{, }
\NormalTok{                           .(scales}\SpecialCharTok{::}\FunctionTok{pvalue}\NormalTok{(res}\SpecialCharTok{$}\NormalTok{pval)), }\StringTok{"; "}\NormalTok{, I}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{, }\StringTok{" = "}\NormalTok{,}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/for-plot2-1.pdf}
...
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/for-plot3-1.pdf}

\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/for-plot3b-1.pdf}
...
Los sesgos de estudios pequeños se producen cuando los estudios más pequeños muestran resultados diferentes (a menudo mayores), que los grandes, lo que puede amenazar la validez de los meta-análisis \autocite{hongTestingSmallStudy2020}.

A pesar de esto, tanto la regresión de Egger como el \emph{funnel plot} son importantes y muy informativos, pues es importante considerar el sesgo que podrían generar estudios con tamaño de muestra pequeño en nuestra estimación del efecto \emph{real} (o, para decirlo más técnicamente, el sesgo que podrían introducir estudios con poder estadístico bajo).
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/funnel-plot1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/funnel-plot1a-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/funnel-plot2-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/funnel-plot3-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/tf-plot1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/tf-plot2-1.pdf}
...
El resultado nos muestra que el análisis aumentó dos estudios hipotéticos a la izquierda. Es decir, sugiere que hay \emph{cierto} riesgo de sesgo de publicación, y de que estemos sobre-estimando un poco el efecto. De hecho, la estimación del efecto al aumentar estos estudios hipotéticos para que la distribución sea realmente simétrica, disminuye a 0.1288, algo menor que el 0.1499 que nos sugería el análisis inicial. Se debe tener en cuenta que los estudios generados por el método \emph{trim and fill}, son hipotéticos. Y que estos estudios se utilizan para estimar el tamaño y la dirección del sesgo de publicación, pero no son datos reales. Por esto, los resultados deben ser interpretados con precaución y no constituyen evidencia definitiva de sesgo de publicación.
...
Al final, está el \emph{Likelihood ratio test} (en español, ``Prueba de la razón de verosimilitud''), que evalúa la bondad del ajuste de dos modelos estadísticos que compiten entre sí, con base en la relación de su verosimilitud\footnote{Sin entrar en detalles respecto a técnicas de inferencia estadística basadas en verosimilitud, si quieres conocer más acerca de estos métodos, así como de estadística Bayesiana (que tiene mucho que ver con verosimilitud) y frecuentista (la que solemos usar, que hace inferencia a partir de valores \(p\)), con énfasis en las fortalezas y limitaciones de cada enfoque, recomiendo el libro \emph{``Improving Your Statistical Inferences''} de \textcite{lakensImprovingYourStatistical2022}. Este libro, gratuito y abierto, integra los contenidos de dos cursos gratuitos del mismo autor (\emph{``\href{https://www.coursera.org/learn/statistical-inferences}{Improving Your Statistical Inferences}''} y \emph{``\href{https://www.coursera.org/learn/improving-statistical-questions}{Improving Your Statistical Questions}''}).}. En este caso, comparando el modelo original, con este modelo con pesos ajustados.
...
Básicamente, esta tabla contiene información que nos permite evaluar si cada estudio incluido en nuestro meta-análisis tenía o no un poder estadístico suficiente para detectar un rango de tamaños de efecto que, como antes van desde \emph{z} = 0.1 (columna \texttt{power\_es01}), hasta \emph{z} = 1.0 (\texttt{power\_es1}). Por ejemplo, el poder estadístico del estudio 13 para detectar un efecto de \emph{z} = 0.1 sería de apenas 0.1861296.

Esta tabla también, nos permite ver el poder estadístico de cada estudio para detectar un tamaño de efecto \emph{real} como el que estimamos (\emph{z} = 0.15), en la columna \texttt{power\_es\_observed} (la información de esta columna es análoga a la de los \emph{funnel plot} creados con la función \texttt{viz\_funnel} del paquete \texttt{metaviz}; Fig. \ref{fig:funnel-plot3}A y B). Por ejemplo, el poder estadístico del estudio 16 para detectar un efecto de \emph{z} = 0.15 sería de 0.2188783.
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/fire-plot1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/fire-plot2-1.pdf}
...
\centering
\begin{tabular}[t]{>{}c>{}r|>{}c>{}r|>{}cr}
\toprule
\multicolumn{1}{c}{\makecell[c]{Número de\\estudios\\(\textit{k})}} & \multicolumn{1}{c}{\makecell[c]{Posibles\\combinaciones\\($2^{k} - 1$)}} & \multicolumn{1}{c}{\makecell[c]{Número de\\estudios\\(\textit{k})}} & \multicolumn{1}{c}{\makecell[c]{Posibles\\combinaciones\\($2^{k} - 1$)}} & \multicolumn{1}{c}{\makecell[c]{Número de\\estudios\\(\textit{k})}} & \multicolumn{1}{c}{\makecell[c]{Posibles\\combinaciones\\($2^{k} - 1$)}}\\
\midrule
\textbf{1} & 1 & \textbf{11} & 2 047 & \textbf{21} & 2 097 151\\
\textbf{2} & 3 & \textbf{12} & 4 095 & \textbf{22} & 4 194 303\\
\textbf{3} & 7 & \textbf{13} & 8 191 & \textbf{23} & 8 388 607\\
\textbf{4} & 15 & \textbf{14} & 16 383 & \textbf{24} & 16 777 215\\
\textbf{5} & 31 & \textbf{15} & 32 767 & \textbf{25} & 33 554 431\\
\textbf{6} & 63 & \textbf{16} & 65 535 & \textbf{26} & 67 108 863\\
\textbf{7} & 127 & \textbf{17} & 131 071 & \textbf{27} & 134 217 727\\
\textbf{8} & 255 & \textbf{18} & 262 143 & \textbf{28} & 268 435 455\\
\textbf{9} & 511 & \textbf{19} & 524 287 & \textbf{29} & 536 870 911\\
\textbf{10} & 1 023 & \textbf{20} & 1 048 575 & \textbf{30} & 1 073 741 823\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/gosh1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/for-plot-mo-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/gosh2-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/gosh3-1.pdf}
...
En este caso, asignaré el resultado a un objeto que llamaré \texttt{gp\_mo\_diag}.
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/reg-plot1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/for-plot-mod1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/funnel-plot-mod1-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/reg-plot2-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/reg-plot3-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/for-plot-mod2-1.pdf}
...
\begin{figure}
\centering
\includegraphics{Meta-analysis_correlaciones_R_v4_files/figure-latex/funnel-plot-mod2-1.pdf}
...
Quiero agradecer especialmente el apoyo de mis grandes amigos y colegas \href{https://scholar.google.es/citations?user=od6gf0wAAAAJ\&hl=es}{Andrés Castellanos-Chacón} y \href{https://www.researchgate.net/profile/Milena-Vasquez-Amezquita}{Milena Vásquez-Amézquita} por su dedicada revisión, correcciones y comentarios a este documento.
...
\begin{ROut}{Consola de R: Output~\thetcbcounter}
                \begin{footnotesize}
                \begin{verbatim} To cite package 'dplyr' in publications use:
...
El código y archivos necesarios para crear este documento, están disponibles en OSF (\url{https://osf.io/na4p9/}). A continuación está la información del sistema, versión de R, y lista de paquetes usados con sus versiones:

\textbf{R version 4.3.2 (2023-10-31)}

\textbf{Platform:} x86\_64-pc-linux-gnu (64-bit)

\textbf{attached base packages:}
\emph{stats}, \emph{graphics}, \emph{grDevices}, \emph{utils}, \emph{datasets}, \emph{methods} and \emph{base}

\textbf{other attached packages:}
\emph{meta(v.6.5-0)}, \emph{dmetar(v.0.1.0)}, \emph{metameta(v.0.2)}, \emph{weightr(v.2.0.2)}, \emph{metaviz(v.0.3.1)}, \emph{tidyquant(v.1.0.7)}, \emph{quantmod(v.0.4.25)}, \emph{TTR(v.0.24.4)}, \emph{PerformanceAnalytics(v.2.0.4)}, \emph{xts(v.0.13.1)}, \emph{zoo(v.1.8-12)}, \emph{psych(v.2.3.12)}, \emph{scales(v.1.3.0)}, \emph{kableExtra(v.1.3.4)}, \emph{ggpubr(v.0.6.0)}, \emph{lubridate(v.1.9.3)}, \emph{forcats(v.1.0.0)}, \emph{stringr(v.1.5.1)}, \emph{dplyr(v.1.1.4)}, \emph{purrr(v.1.0.2)}, \emph{readr(v.2.1.4)}, \emph{tidyr(v.1.3.0)}, \emph{tibble(v.3.2.1)}, \emph{ggplot2(v.3.4.4)}, \emph{tidyverse(v.2.0.0)}, \emph{metafor(v.4.4-0)}, \emph{numDeriv(v.2016.8-1.1)}, \emph{metadat(v.1.2-0)}, \emph{Matrix(v.1.6-3)}, \emph{robumeta(v.2.1)} and \emph{knitr(v.1.45)}

\textbf{loaded via a namespace (and not attached):}
\emph{mnormt(v.2.1.1)}, \emph{pbapply(v.1.7-2)}, \emph{gridExtra(v.2.3)}, \emph{Quandl(v.2.11.0)}, \emph{rlang(v.1.1.2)}, \emph{magrittr(v.2.0.3)}, \emph{compiler(v.4.3.2)}, \emph{flexmix(v.2.3-19)}, \emph{mgcv(v.1.9-1)}, \emph{systemfonts(v.1.0.5)}, \emph{vctrs(v.0.6.5)}, \emph{quadprog(v.1.5-8)}, \emph{rvest(v.1.0.3)}, \emph{pkgconfig(v.2.0.3)}, \emph{fastmap(v.1.1.1)}, \emph{backports(v.1.4.1)}, \emph{magic(v.1.6-1)}, \emph{labeling(v.0.4.3)}, \emph{pander(v.0.6.5)}, \emph{utf8(v.1.2.4)}, \emph{rmarkdown(v.2.25)}, \emph{tzdb(v.0.4.0)}, \emph{nloptr(v.2.0.3)}, \emph{modeltools(v.0.2-23)}, \emph{xfun(v.0.41)}, \emph{jsonlite(v.1.8.8)}, \emph{highr(v.0.10)}, \emph{fpc(v.2.2-11)}, \emph{prabclus(v.2.3-3)}, \emph{cluster(v.2.1.6)}, \emph{broom(v.1.0.5)}, \emph{parallel(v.4.3.2)}, \emph{R6(v.2.5.1)}, \emph{stringi(v.1.8.3)}, \emph{RColorBrewer(v.1.1-3)}, \emph{car(v.3.1-2)}, \emph{boot(v.1.3-28)}, \emph{diptest(v.0.77-0)}, \emph{Rcpp(v.1.0.11)}, \emph{bookdown(v.0.37)}, \emph{poibin(v.1.5)}, \emph{nnet(v.7.3-19)}, \emph{splines(v.4.3.2)}, \emph{timechange(v.0.2.0)}, \emph{tidyselect(v.1.2.0)}, \emph{rstudioapi(v.0.15.0)}, \emph{abind(v.1.4-5)}, \emph{yaml(v.2.3.8)}, \emph{MuMIn(v.1.47.5)}, \emph{codetools(v.0.2-19)}, \emph{curl(v.5.2.0)}, \emph{lattice(v.0.22-5)}, \emph{withr(v.2.5.2)}, \emph{evaluate(v.0.23)}, \emph{CompQuadForm(v.1.4.3)}, \emph{xml2(v.1.3.6)}, \emph{kernlab(v.0.9-32)}, \emph{mclust(v.6.0.1)}, \emph{pillar(v.1.9.0)}, \emph{carData(v.3.0-5)}, \emph{stats4(v.4.3.2)}, \emph{generics(v.0.1.3)}, \emph{mathjaxr(v.1.6-0)}, \emph{hms(v.1.1.3)}, \emph{munsell(v.0.5.0)}, \emph{minqa(v.1.2.6)}, \emph{class(v.7.3-22)}, \emph{glue(v.1.6.2)}, \emph{tools(v.4.3.2)}, \emph{robustbase(v.0.99-1)}, \emph{lme4(v.1.1-35.1)}, \emph{webshot(v.0.5.5)}, \emph{ggsignif(v.0.6.4)}, \emph{cowplot(v.1.1.2)}, \emph{grid(v.4.3.2)}, \emph{netmeta(v.2.8-2)}, \emph{colorspace(v.2.1-0)}, \emph{nlme(v.3.1-163)}, \emph{cli(v.3.6.2)}, \emph{fansi(v.1.0.6)}, \emph{ggthemes(v.5.0.0)}, \emph{viridisLite(v.0.4.2)}, \emph{svglite(v.2.1.3)}, \emph{DEoptimR(v.1.1-3)}, \emph{gtable(v.0.3.4)}, \emph{rstatix(v.0.7.2)}, \emph{digest(v.0.6.33)}, \emph{ggrepel(v.0.9.4)}, \emph{farver(v.2.1.1)}, \emph{htmltools(v.0.5.7)}, \emph{lifecycle(v.1.0.4)}, \emph{httr(v.1.4.7)} and \emph{MASS(v.7.3-60)}
