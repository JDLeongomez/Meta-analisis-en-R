...
knitr::include_graphics("images/Teoria-meta_FIN.pdf")
```
...
El modelo de función de peso de Vevea y Hedges [-@veveaGeneralLinearModel1995; ver también @veveaPublicationBiasResearch2005] es uno de los modelos se selección (en inglés, *selection models*) más conocidos[^9].
...
Adicionalmente, incluiré el argumento `db.params = list(eps = 0.1, MinPts = 50)`, para especificar parámetros para el algoritmo *DBSCAN*, pues los *clusters* requieren un número mínimo de puntos (`MinPts`) dentro de una distancia máxima (`eps`) alrededor de uno de sus miembros (o semilla). En este caso, agregué `eps = 0.1` para especificar que la "distancia de *alcanzabilidad*" (en inglés, *reachability distance*) es de 0.1, y `MinPts = 50` determinar el número mínimo de puntos necesarios para cada *cluster* (en este caso, 50). Si quieres información detallada y profunda sobre estos parámetros, te recomiendo leer @esterAlgorithm.
...
```{r fancy-gosh1, echo = FALSE, out.height = "200px", fig.align = "center", fig.cap = "\\textit{GOSH plot}, desequilibrio de \\textit{clusters} y distancia de Cook estimados usando el algoritmo \\textit{K-means}. Los \\textit{clusters} identificados se representan con diferentes colores, y sus efectos se muestran en los 3 paneles. Este algoritmo identificó 2 \\textit{clusters}, pues así fue especificado."}
...
```{r fancy-gosh3, echo = FALSE, out.height = "200px", fig.align = "center", fig.cap = "\\textit{GOSH plot}, desequilibrio de \\textit{clusters} y distancia de Cook estimados usando el algoritmo de modelos de mezcla gaussiana (en inglés, \\textit{Gaussian Mixture Model} o \\textit{GMM} por sus siglas). Los \\textit{clusters} identificados se representan con diferentes colores, y sus efectos se muestran en los 3 paneles. Este algoritmo identificó 9 \\textit{clusters}."}
...
Primero, y como ejemplo, vamos a ver si la edad promedio de los participantes de un estudio (en nuestros datos, `meanage`) modera el resultado. Esto es importante, pues hay una enorme variación entre las edades medias de los participantes de los diferentes estudios[^15], lo que podría moderar (afectar) la asociación entre escrupulosidad (*conscientiousness*) y adherencia a la medicación prescrita.

[^15]: De hecho, mientras que en el estudio de `r paste0(dat$authors[which.min(dat$meanage)], " (", dat$year[which.min(dat$meanage)], ")")` la edad promedio fue de `r min(dat$meanage)`, en el estudio de `r paste0(dat$authors[which.max(dat$meanage)], " (", dat$year[which.max(dat$meanage)], ")")` la edad promedio fue de `r max(dat$meanage)`.
...
Primero, debes tener hacer una selección de artículos buena y no sesgada[^20]. Esto mínimamente incluye:

[^20]: Para esto existen varias guías, e incluso la *Declaración PRISMA* que está diseñada para abordar los problemas en la publicación de revisiones sistemáticas y meta-análisis [para información en español, ver e.g., @huttonExtensionDeclaracionPRISMA2016; @pageDeclaracionPRISMA20202021].
...
Como ejemplo, en ésta guía usé `dplyr` [@WickhamDplyr2021] para manipular los datos. Usando la función `citation`:
