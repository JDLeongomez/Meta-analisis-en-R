...
La función `pcurve` produce varios elementos relacionados la curva de valores $p$ (*p-curve*), incluyendo la Figura \@ref(fig:pcurve-plot) que resume éste análisis.

```{r pcurve-plot, echo = FALSE, fig.cap = "Curva de valores $p$ (*p-curve*) creada con [`dmetar`](http://dmetar.protectlab.org/). Para una entender una curva de valores $p$, su relevancia y su interpretación, sugiero leer Simonsohn et al. (2019, 2014), o ver el sitio web del paquete `dmetar`: [http://dmetar.protectlab.org/](http://dmetar.protectlab.org/)."}
